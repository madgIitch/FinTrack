...
if (storedFirst || storedLast) updateWelcomeMessage(storedFirst, storedLast);
//
...
async function buildAccountMap(userId) {
    ...
    const map = {};
    for (const { accessToken } of raw)try {
...
(0, _auth.onAuthStateChanged)((0, _firebaseJs.auth), async (user)=>{
    ...
    if (user) {
        ...
        try {
            const uref = (0, _firestore.doc)((0, _firebaseJs.db), 'users', user.uid);
